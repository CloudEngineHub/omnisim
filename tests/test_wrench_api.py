...
def _binary():
    """The engine to run. $OMNISIM_BINARY first (the same override the harness
    and run-headless honour), so a source clone with no built engine can be
    tested against an installed one."""
...
pytestmark = pytest.mark.skipif(_binary() is None,
                                reason="no omnisim-bin in this clone; set "
                                       "$OMNISIM_BINARY to an installed engine")
    ...
    try:
        ...
        if proc.poll() is None:
            ...
            try:
                ...
                proc.kill()
        # The world has to live beside tests/physics/controllers/ for the
        # engine to resolve wrench_probe, so it cannot go in tmp_path; do not
        # leave it behind as an untracked file in the tree.
        try:
            world.unlink()
        except OSError:
            pass
