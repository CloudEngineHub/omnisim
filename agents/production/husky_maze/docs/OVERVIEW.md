...
- **Skid-steer pivots drifted** — 90° pivots accumulated ~0.5 m of body translation per turn, and we added `snap_to_cell` to teleport the husky back onto the grid after each step. ⚠️ **Both the mechanism and the workaround are historical.** The mechanism was never "wheel friction": the solver bounded a velocity motor's gain at `kv <= M_ii/dt`, so a wheel's stall torque tracked its own rotational inertia instead of the URDF's declared effort, and a pivot — the first real load a wheeled base meets — could not generate the traction force it needed, so the base **slid instead of pivoting**. Fixed 2026-09-11 (`69b4b024b`); open loop the Husky's chassis yaw ratio went 0.0069 → 0.532. `snap_to_cell` was removed before that (it returns 410) and the husky now navigates entirely under wheel control; moves are verified against the cell centre instead of teleported onto it.
