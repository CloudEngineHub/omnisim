...
- ⚠️ **A collision mesh the reader cannot decode is now an ERROR, not a silent placeholder sphere (2026-09-11).** OmniSim reads meshes through assimp 5.2.3, vendored as a **prebuilt** library per platform (`dependencies/assimp-5.2.3.zip` ships `msys64/mingw64/bin/libassimp-5.dll`; Linux/macOS pull their own from the `deps-*-v1` mirror tags). That build has **no Draco decoder**, so any glTF/GLB that lists `KHR_draco_mesh_compression` in `extensionsRequired` is refused **whole** — assimp returns no scene and logs `GLTF: Draco mesh compression not supported`. Measured 2026-09-11 on the public urdfeus gallery (718 models, `github.com/iory/urdfeus` @ `a5d094b`): **all 4,024 of its `.glb` meshes are Draco**, so every `<collision><mesh>` in it reached the solver as a **12 cm placeholder sphere** — and the world still loaded, still stepped, and still exited 0. `EXT_meshopt_compression` is worse rather than better: assimp does not refuse it, it reads the encoded buffers as if they were plain vertex data, so the geometry is garbage instead of absent. A body whose declared mesh collider silently became a sphere makes every contact, grasp and resting pose it produces meaningless, so it now raises an `ERROR` naming the Solid and the mesh URL, which **FAILs a bare `run-headless`** without `--fail-on-warning`. Value-parsed hatch `OMNISIM_STRICT_COLLISION_MESH=0` downgrades it to a warning for anyone who has to run such a world while the mesh is being converted. ✅ **Catch it before the run:** `python scripts/dev/urdf_import.py <robot.urdf> --report r.json --strict` now distinguishes *missing* from *present-but-undecodable* and exits 2 on either; its old check only asked whether a file existed, and reported the whole 718-model gallery clean. `python -m omnisim validate-urdf <robot.urdf> --check-meshes` reports the same thing as an `assets/mesh_undecodable` finding — that tier could not produce a finding at all before 2026-09-11, because it skipped every reference whose resolved path was empty, which is exactly how an unresolvable reference is spelled. The fix for an affected asset is to re-export the mesh uncompressed (glTF/STL/OBJ/DAE) or to author an explicit primitive `boundingObject`.
