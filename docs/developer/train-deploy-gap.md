...
|---|---|---|---|---|
| 1 | **Link COM** | true URDF COM (`add_urdf`) | **true URDF COM since 2026-09-10** — a declared `inertiaMatrix` is by definition about the `centerOfMass`, so `OmSolid::addBody` now passes the two together (`OMNISIM_NEWTON_INERTIA_COM=0` reverts). `OMNISIM_NEWTON_USE_LINK_COM` still governs only the geometry-derived case. | **CLOSED BY DEFAULT, AND IT INVALIDATES THE OLD CHAMPION.** Measured on `g1_stand_deploy` the same day: 14 of 15 bodies moved, max \|Δ`body_ipos`\| = **0.154 m** — exactly the residual `g1_physics.json._residuals.deploy_link_com` called *"the dominant cause of trajectory drift"*. Deploy now matches the trainer. Every policy trained against the old COM-at-origin deploy **must be retrained** (the old walker was predicted to fall ~11.7 s with this flip); set `OMNISIM_NEWTON_INERTIA_COM=0` to run one on the physics it was trained on. |
