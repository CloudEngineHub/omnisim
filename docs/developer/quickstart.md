...
- **Using your own venv on top of the bootstrap** (public issue #17): the `gpu` phase installs warp / newton / torch into the *system* interpreter on purpose, because that is the one the engine embeds and a venv is invisible to it. A venv you create for your own tooling therefore does not see those wheels until you let it fall through to the system site-packages: `python3 -m venv --system-site-packages .venv`, or set `include-system-site-packages = true` in an existing venv's `pyvenv.cfg`. Do not re-install the physics wheels into the venv instead; the engine will never import them from there.
- **`python -m omnisim` runs from the repository root.** The clone directory is itself named `omnisim` and contains the `omnisim` package, so from the *parent* directory Python resolves the clone as an empty namespace package and reports `No module named omnisim.__main__`. `cd` into the clone first; every command in this guide assumes that. There is no installable package yet, so `pip install -e .` is not an alternative.
